...
var fs = require('fs'),
    ...
    var k = function(options){
        var t = this;
        t.data = options || {};
        if('rewatch' in t.data && t.data.rewatch == true){
            t.rewatch();
        }
        t.watch();
        return t;
                    ...
                    if(!('files' in job)){
                        ...
                        job.files.forEach(function(file){
                            try{
                                ...
                                winston.warn('Unable to stop watching file located at: ' + file);
                            }
                        });
                    }
                });
    ...
    k.prototype.rewatch = function(){
        var t = this;
        try{
            fs.watchFile('./.krunch', {persistent: true, interval: 0}, function(curr, prev){
                if(curr.mtime.getTime() !== prev.mtime.getTime()){
                    try{
                        t.data = JSON.parse(fs.readFileSync('./.krunch', 'utf8'));
                        try{
                            t.unwatch();
                            t.watch();
                        }
                        catch(e){
                            winston.error('Couldn\'t rewatch :(');
                        }
                    }
                    catch(e){
            ...
            winston.warn('Unable to find .krunch, will not rewatch :(');
        }
    };
