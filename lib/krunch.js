...
// TODO: Allow for development and production based configs/vars for static urls and other vars that may need to be set? Like a small time templating solution
var fs = require('fs'),
    util = require('util'),
    path = require('path'),
    less = require('less'),
    ...
    var k = function(config){
        var t = this;
            ...
            t.config_file = null;
        if(config || false){
            if(typeof config == 'string' && config.substr(0,1) != '{'){
                // Assume filename
                t.load(config);
                t.watch();
            }
        ...
        ['html', 'js', 'less'].forEach(function(type){
            ...
            c[type].forEach(function(job){
                if(!('files' in job)){
                    ...
                    job = extend(true, {}, c.default[type], job);
                    var files = [];
    ...
    k.prototype.less = function(j){
        ...
        var parser = new(less.Parser)({
            paths: j.import_paths,
            filename: path.basename(file_out)
        });
        try{
